...
func (r *ProtocolOCI) GetImageFile(fingerprint string, req ImageFileRequest) (*ImageFileResponse, error) {
	...
	// Turn into a tarball.
	_, err = subprocess.RunCommand("tar", "-Jcf", filepath.Join(ociPath, "rootfs.tar.xz"), "-C", filepath.Join(ociPath, "image", "rootfs"), ".")
	if err != nil {
		return nil, err
	}

	err = os.RemoveAll(filepath.Join(ociPath, "image", "rootfs"))
	if err != nil {
		return nil, err
	}

	_, err = subprocess.RunCommand("tar", "-Jcf", filepath.Join(ociPath, "metadata.tar.xz"), "-C", filepath.Join(ociPath, "image"), ".")
	if err != nil {
		return nil, err
	}

	err = os.RemoveAll(filepath.Join(ociPath, "image"))
	if err != nil {
		return nil, err
	}

	// Copy the data.
	f, err := os.Open(filepath.Join(ociPath, "metadata.tar.xz"))
	if err != nil {
		return nil, err
	}
	...
	size, err := io.Copy(req.MetaFile, f)
	if err != nil {
		return nil, err
	}
	...
	f, err = os.Open(filepath.Join(ociPath, "rootfs.tar.xz"))
	if err != nil {
		return nil, err
	}
	...
	size, err = io.Copy(req.RootfsFile, f)
	if err != nil {
		return nil, err
	}
