...
// GetInstanceDirectBackupFile requests that Incus creates and returns new direct backup for the
// instance.
func (r *ProtocolIncus) GetInstanceDirectBackupFile(instanceName string, backup api.InstanceBackupsPost, req *BackupFileRequest) error {
	path, _, err := r.instanceTypeToPath(api.InstanceTypeAny)
	if err != nil {
		return err
	}
