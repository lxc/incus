...
You can obtain the list of TLS certificates trusted by a LXD server with [`lxc config trust list`](incus_config_trust_list.md).
...
1. When the user adds a server with [`lxc remote add`](incus_remote_add.md), the server is contacted over HTTPS, its certificate is downloaded and the fingerprint is shown to the user.
...
To revoke trust to a client, remove its certificate from the server with [`lxc config trust remove <fingerprint>`](incus_config_trust_remove.md).
...
To restrict access, use [`lxc config trust edit <fingerprint>`](incus_config_trust_edit.md).
...
To do so, copy the client certificate to the server and register it using [`lxc config trust add <file>`](incus_config_trust_add.md).
...
To use this method, generate a token for each client by calling [`lxc config trust add`](incus_config_trust_add.md), which will prompt for the client name.
...
Alternatively, the clients can provide the token directly when adding the remote: [`lxc remote add <name> <token>`](incus_remote_add.md).
...
To add a remote pointing to a LXD server configured with OIDC authentication, run [`lxc remote add <remote_name> <remote_address>`](incus_remote_add.md).
