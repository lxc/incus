...
To set an instance option for a profile, use the [`lxc profile set`](incus_profile_set.md) command.
...
To add and configure an instance device for your profile, use the [`lxc profile device add`](incus_profile_device_add.md) command.
...
To configure instance device options for a device that you have added to the profile earlier, use the [`lxc profile device set`](incus_profile_device_set.md) command:
...
```{tip}
Check the configuration after adding the profile: [`lxc config show <instance_name>`](incus_config_show.md)
