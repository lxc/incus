...
-|-
truenas.allow-insecure | specifies --allow-insecure for truenas tool
truenas.api_key | specifies --api-key for the truenas tool
truenas.host | specifies --host for the truenas tool
truenas.config | specifies --config for the truenas tool
truenas.config-file | specifies --config-file for the truenas tool
truenas.dataset | the remote dataset on the host

host/dataset and url can easily be generated by specifying a source using the format: `[<remote host>:]<remote pool>[[/<remote dataset>]...][/]`

The remote dataset will be created if it does not exist.
