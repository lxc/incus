...
Setting [`zfs.delegate`](storage-zfs-vol-config) on a container volume delegates its {spellexception}`dataset` to the container.
The container can then use the `zfs` command on that {spellexception}`dataset`, for example to create, snapshot or mount its own {spellexception}`datasets` underneath it.

Mount those {spellexception}`datasets` from inside the container, either with `zfs mount` or through `/etc/fstab` when using `mountpoint=legacy`.
Mount units generated by `zfs-mount-generator` are not available for delegated {spellexception}`datasets`.
