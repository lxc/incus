...
type Stmt struct {
	...
	dbPkg  *types.Package    // Package where database related helpers are located.
...
func NewStmt(database, pkg, entity, kind string, config map[string]string) (*Stmt, error) {
	...
	if database != "" {
		...
		dbPkgs, err := packages.Load(&packages.Config{
...
func (s *Stmt) objectsBy(buf *file.Buffer) error {
	...
	sqlString, err := ParseStmt(stmtCodeVar(s.entity, "objects"), s.defs)
	if err != nil {
		return err
	}
...
func (s *Stmt) create(buf *file.Buffer, replace bool) error {
	...
	for _, field := range all {
		column, value, err := field.InsertColumn(s.pkg, s.dbPkg, mapping, table, s.defs)
		if err != nil {
			return err
		}
