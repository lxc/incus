...
func instanceSnapRestore(s *state.State, projectName string, name string, snap string, stateful bool, diskOnly bool, op *operations.Operation) error {
	...
	// Ensure restoring the snapshot's config doesn't violate project restrictions, adjusting its volatile.* keys as needed.
	...
	err = s.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		return projecthelpers.AllowSnapshotRestore(tx, projectName, name, api.InstancePut{
