...
func bucketBackupCreate(s *state.State, args db.StoragePoolBucketBackup, projectName string, poolName string, bucketName string, keyName string) error {
	...
	// Create the database entry
	err = s.DB.Cluster.CreateStoragePoolBucketBackup(args)
	if err != nil {
		if err == db.ErrAlreadyDefined {
			return fmt.Errorf("Backup %q already exists", args.Name)
		}
	...
	revert.Add(func() { _ = s.DB.Cluster.DeleteStoragePoolBucketBackup(args.Name) })

	backupRow, err := s.DB.Cluster.GetStoragePoolBucketBackup(projectName, poolName, args.Name)
	if err != nil {
		return fmt.Errorf("Failed getting backup record: %w", err)
	}
	...
	err = pool.BackupBucket(projectName, bucketName, keyName, tarWriter, nil)
	if err != nil {
		return fmt.Errorf("Backup create: %w", err)
	}
