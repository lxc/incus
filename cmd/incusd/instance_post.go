		...
		if req.Pool != "" || req.Project != "" || target != "" {
			...
			if req.Pool != "" {
				if inst.Type() != instancetype.VM {
					return response.BadRequest(errors.New("Storage pool change supported only by virtual-machines"))
				}

				if target == "" {
					return response.BadRequest(errors.New("Storage pool can be specified only together with target flag"))
				}
			}
