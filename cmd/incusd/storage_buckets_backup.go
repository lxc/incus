...
func storagePoolBucketBackupsPost(d *Daemon, r *http.Request) response.Response {
	...
	if req.Name == "" {
		// come up with a name.
		backups, err := s.DB.Cluster.GetStoragePoolBucketBackupsName(projectName, bucketName)
		if err != nil {
			return response.BadRequest(err)
		}
	...
	backup := func(op *operations.Operation) error {
		...
		err := bucketBackupCreate(s, args, projectName, poolName, bucketName, req.KeyName)
		if err != nil {
...
func storagePoolBucketBackupLoadByName(s *state.State, projectName, poolName, backupName string) (*backup.BucketBackup, error) {
	b, err := s.DB.Cluster.GetStoragePoolBucketBackup(projectName, poolName, backupName)
	if err != nil {
		return nil, err
	}
