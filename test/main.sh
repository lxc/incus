...
# Disk, volume, quota, and filesystem management
...
# Concerns IP, communication protocols, and network devices
...
# Mechanisms that prevent attacks, privilege escalation, or unauthorized access
run_standalone_security() {
...
run_suite_category() {
    s="$1"
    c="$2"
    test_category="run_${s}_${c}"

    if type "$test_category" >/dev/null 2>&1; then
        $test_category
    else
        echo "Unknown test category: $s $c"
        exit 1
    fi
}
...
case "$suite" in
  ...
  standalone)
    if [ "$category" = "all" ]; then
        ...
        run_suite_category standalone "$category"
    fi
    ;;
  cluster)
    if [ "$category" = "all" ]; then
        ...
        run_suite_category cluster "$category"
    fi
    ;;
  *)
