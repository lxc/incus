test_container_devices_nic_p2p() {
  ...
  inc profile device set ${ctName} eth0 nictype "p2p"
  ...
  inc exec "${ctName}" -- ip -6 route add default dev eth0
  wait_for_dad "${ctName}" eth0
  ...
  inc config device set "${ctName}" eth0 ipv4.routes "192.0.2.3${ipRand}/32"
  inc config device set "${ctName}" eth0 ipv6.routes "2001:db8::3${ipRand}/128"
  ...
  inc config device add "${ctName}" eth1 nic nictype=p2p
