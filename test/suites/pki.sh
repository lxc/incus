test_pki() {
  ...
  (
    ...
    ! inc_remote remote add pki-incus "${INCUS5_ADDR}" --accept-certificate --password=bar || false

    # Add remote using the correct password.
    # This should work because the client certificate is signed by the CA.
    inc_remote remote add pki-incus "${INCUS5_ADDR}" --accept-certificate --password=foo
    inc_remote config trust ls pki-incus: | grep incus-client
    inc_remote remote remove pki-incus
    ...
    INCUS_DIR=${INCUS5_DIR} inc config set core.trust_ca_certificates true
    inc_remote remote add pki-incus "${INCUS5_ADDR}" --accept-certificate
    inc_remote config trust ls pki-incus: | grep incus-client
    inc_remote remote remove pki-incus
    ...
    inc_remote remote add pki-incus "${INCUS5_ADDR}" --accept-certificate --password=bar
    inc_remote config trust ls pki-incus: | grep incus-client

    # Try removing the fingerprint.
    # This should succeed as the admin can delete all certificates.
    fingerprint="$(inc_remote config trust ls pki-incus: --format csv | cut -d, -f4)"
    inc_remote config trust rm pki-incus:"${fingerprint}"
    ...
    ! inc_remote remote add pki-incus "${INCUS5_ADDR}" --accept-certificate --password=foo || false
    ...
    ! inc_remote remote add pki-incus "${INCUS5_ADDR}" --accept-certificate --password=incorrect || false
  )

  # Confirm that a normal, non-PKI certificate doesn't.
  # As INCUS_CONF is not set to INC5_DIR where the CA signed client certs are, this will cause the inc command to
