...
EOF
  ...
  INCUS_DIR="${INCUS_ONE_DIR}" lxc init testimage c1 -c limits.memory=512MiB -c limits.cpu=1 -d root,size=200MiB

  # Create instances with limits set on a profile to test expansion and to appease intance_resources checks.
  INCUS_DIR="${INCUS_TWO_DIR}" lxc profile create foo
  INCUS_DIR="${INCUS_TWO_DIR}" lxc profile show default | lxc profile edit foo
  INCUS_DIR="${INCUS_TWO_DIR}" lxc profile set foo limits.cpu=1 limits.memory=512MiB
  INCUS_DIR="${INCUS_TWO_DIR}" lxc profile device set foo root size=200MiB
  INCUS_DIR="${INCUS_TWO_DIR}" lxc init testimage c2 -p foo
  INCUS_DIR="${INCUS_THREE_DIR}" lxc init testimage c3 -p foo
  INCUS_DIR="${INCUS_ONE_DIR}" lxc info c1 | grep -q "Location: node2"
  INCUS_DIR="${INCUS_ONE_DIR}" lxc info c2 | grep -q "Location: node2"
  INCUS_DIR="${INCUS_ONE_DIR}" lxc info c3 | grep -q "Location: node2"
  INCUS_DIR="${INCUS_ONE_DIR}" lxc delete -f c1 c2 c3
  INCUS_DIR="${INCUS_ONE_DIR}" lxc profile delete foo
...
EOF
  ...
  ! INCUS_DIR="${INCUS_ONE_DIR}" lxc init testimage c1 || false
...
EOF
  ...
  INCUS_DIR="${INCUS_ONE_DIR}" lxc init testimage c1 -c cluster.evacuate=migrate
...
EOF
  ...
  INCUS_DIR="${INCUS_ONE_DIR}" lxc cluster evacuate node1 --force
  INCUS_DIR="${INCUS_ONE_DIR}" lxc cluster show node1 | grep -q "status: Evacuated"
