test_image_acl() {
  ...
  CONTAINER_PID="$(inc query /1.0/instances/c1?recursion=1 | jq '.state.pid')"
  inc exec c1 touch foo
  ...
  inc stop c1
  inc publish c1 --alias c1-with-acl

  # Launch a new container from the existing image
  inc launch c1-with-acl c2

  # Check if the ACLs are still present
  CONTAINER_PID="$(inc query /1.0/instances/c2?recursion=1 | jq '.state.pid')"
