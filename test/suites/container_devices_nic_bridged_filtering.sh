test_container_devices_nic_bridged_filtering() {
  ...
  inc network set "${brName}" ipv4.routing true
  inc network set "${brName}" ipv6.routing true
  ...
  inc network set "${brName}" ipv6.address 2001:db8:1::1/64
  ...
  inc profile show "${ctPrefix}" | sed  "s/nictype: p2p/nictype: bridged\\n    parent: ${brName}/" | inc profile edit "${ctPrefix}"

  # Launch first container.
  inc init testimage "${ctPrefix}A" -p "${ctPrefix}"
  inc config device add "${ctPrefix}A" eth0 nic nictype=nic name=eth0 nictype=bridged parent="${brName}"
  inc start "${ctPrefix}A"
  inc exec "${ctPrefix}A" -- ip a add 192.0.2.2/24 dev eth0

  # Launch second container.
  inc init testimage "${ctPrefix}B" -p "${ctPrefix}"
  inc config device add "${ctPrefix}B" eth0 nic nictype=nic name=eth0 nictype=bridged parent="${brName}"
  inc start "${ctPrefix}B"
  inc exec "${ctPrefix}B" -- ip a add 192.0.2.3/24 dev eth0

  # Check basic connectivity without any filtering.
  inc exec "${ctPrefix}A" -- ping -c2 -W5 192.0.2.1
  inc exec "${ctPrefix}A" -- ping -c2 -W5 192.0.2.3

  # Enable MAC filtering on CT A and test.
  inc config device set "${ctPrefix}A" eth0 security.mac_filtering true
  ctAMAC=$(inc config get "${ctPrefix}A" volatile.eth0.hwaddr)

  # Check MAC filter is present in firewall.
  ctAHost=$(inc config get "${ctPrefix}A" volatile.eth0.host_name)
  if [ "$firewallDriver" = "xtables" ]; then
  ...
  inc exec "${ctPrefix}A" -- ip link set dev eth0 address 00:11:22:33:44:56 up

  # Check that ping is no longer working (i.e its filtered after fake MAC setup).
  if inc exec "${ctPrefix}A" -- ping -c2 -W5 192.0.2.1; then
      echo "MAC filter not working to host"
      false
  fi

  # Check that ping is no longer working (i.e its filtered after fake MAC setup).
  if inc exec "${ctPrefix}A" -- ping -c2 -W5 192.0.2.3; then
      echo "MAC filter not working to other container"
      false
  fi

  # Restore real MAC
  inc exec "${ctPrefix}A" -- ip link set dev eth0 address "${ctAMAC}" up
  ...
  inc start "${ctPrefix}A"
  inc exec "${ctPrefix}A" -- ip link set dev eth0 address "${ctAMAC}" up
  inc exec "${ctPrefix}A" -- ip a add 192.0.2.254/24 dev eth0
  inc exec "${ctPrefix}A" -- ping -c2 -W5 192.0.2.1
  inc exec "${ctPrefix}A" -- ping -c2 -W5 192.0.2.3

  # Enable IPv4 filtering on CT A and test (disable security.mac_filtering to check its applied too).
  inc config device set "${ctPrefix}A" eth0 ipv4.address 192.0.2.2
  inc config device set "${ctPrefix}A" eth0 security.mac_filtering false
  inc config device set "${ctPrefix}A" eth0 security.ipv4_filtering true
  inc config device set "${ctPrefix}A" eth0 ipv4.routes 198.51.100.0/24
  inc config device set "${ctPrefix}A" eth0 ipv4.routes.external 203.0.113.0/24
  ...
  inc exec "${ctPrefix}A" -- ip link set dev eth0 address "${ctAMAC}" up
  inc exec "${ctPrefix}A" -- udhcpc -f -i eth0 -n -q -t5
  inc exec "${ctPrefix}A" -- ip a flush dev eth0
  inc exec "${ctPrefix}A" -- ip a add 192.0.2.2/24 dev eth0
  ...
  inc exec "${ctPrefix}A" -- ip a flush dev eth0
  inc exec "${ctPrefix}A" -- ip a add 198.51.100.1/32 dev eth0
  inc exec "${ctPrefix}A" -- ip r add 192.0.2.0/24 dev eth0
  inc exec "${ctPrefix}B" -- ip r add 198.51.100.0/24 dev eth0
  ...
  inc exec "${ctPrefix}A" -- ip a flush dev eth0
  inc exec "${ctPrefix}A" -- ip a add 203.0.113.1/32 dev eth0
  inc exec "${ctPrefix}A" -- ip r add 192.0.2.0/24 dev eth0
  inc exec "${ctPrefix}B" -- ip r add 203.0.113.0/24 dev eth0
  ...
  inc config device unset "${ctPrefix}A" eth0 ipv4.address
  inc start "${ctPrefix}A"
  ...
  inc stop -f "${ctPrefix}A"
  inc config device set "${ctPrefix}A" eth0 security.ipv4_filtering false
  rm "${INCUS_DIR}/networks/${brName}/dnsmasq.hosts/${ctPrefix}A.eth0"
  ...
  inc stop -f "${ctPrefix}A"
  inc network set "${brName}" ipv4.dhcp.ranges "192.0.2.100-192.0.2.110"
  inc start "${ctPrefix}A"
  ...
  inc exec "${ctPrefix}B" -- ip -6 a add 2001:db8:1::3/64 dev eth0
  wait_for_dad "${ctPrefix}B" eth0

  inc exec "${ctPrefix}A" -- ip link set dev eth0 address "${ctAMAC}" up
  inc exec "${ctPrefix}A" -- ip -6 a add 2001:db8:1::254 dev eth0
  wait_for_dad "${ctPrefix}A" eth0
  inc exec "${ctPrefix}A" -- ping6 -c2 -W5 2001:db8:1::1
  inc exec "${ctPrefix}A" -- ping6 -c2 -W5 2001:db8:1::3

  # Enable IPv6 filtering on CT A and test (disable security.mac_filtering to check its applied too).
  inc config device set "${ctPrefix}A" eth0 ipv6.address 2001:db8:1::2
  inc config device set "${ctPrefix}A" eth0 security.mac_filtering false
  inc config device set "${ctPrefix}A" eth0 security.ipv6_filtering true

  # Set the ipv6.routes with mask ::ffff (i.e. subnet is last four hex digits)
  inc config device set "${ctPrefix}A" eth0 ipv6.routes 2001:db8:2::/64
  inc config device set "${ctPrefix}A" eth0 ipv6.routes.external 2001:db8:3::/64
  ...
  inc exec "${ctPrefix}A" -- ip -6 a flush dev eth0
  inc exec "${ctPrefix}A" -- ip -6 a add 2001:db8:1::2/64 dev eth0
  wait_for_dad "${ctPrefix}A" eth0
  ...
  # Add a fake IP
  inc exec "${ctPrefix}A" -- ip -6 a flush dev eth0
  inc exec "${ctPrefix}A" -- ip -6 a add 2001:db8:1::254/64 dev eth0
  wait_for_dad "${ctPrefix}A" eth0

  # Check that ping is no longer working (i.e its filtered after fake IP setup).
  if inc exec "${ctPrefix}A" -- ping6 -c2 -W5 2001:db8:1::1; then
      echo "IPv6 filter not working to host"
      false
  fi

  # Check that ping is no longer working (i.e its filtered after fake IP setup).
  if inc exec "${ctPrefix}A" -- ping6 -c2 -W5 2001:db8:1::3; then
  ...
  inc exec "${ctPrefix}A" -- ip -6 a flush dev eth0
  inc exec "${ctPrefix}A" -- ip -6 a add 2001:db8:2::1/128 dev eth0
  inc exec "${ctPrefix}A" -- ip -6 r add 2001:db8:1::/64 dev eth0
  inc exec "${ctPrefix}B" -- ip -6 r add 2001:db8:2::/64 dev eth0
  wait_for_dad "${ctPrefix}A" eth0
  ...
  inc exec "${ctPrefix}A" -- ip -6 a flush dev eth0
  inc exec "${ctPrefix}A" -- ip -6 a add 2001:db8:3::1/128 dev eth0
  inc exec "${ctPrefix}B" -- ip -6 r add 2001:db8:3::/64 dev eth0
  wait_for_dad "${ctPrefix}A" eth0
  ...
  if inc config show "${ctPrefix}A" | grep volatile.eth0 | grep -v volatile.eth0.hwaddr ; then
    echo "unexpected volatile key remains"
    false
  fi

  # Set static MAC so that SLAAC address is derived predictably and check it is applied to static config.
  inc config device unset "${ctPrefix}A" eth0 ipv6.address
  inc config device set "${ctPrefix}A" eth0 hwaddr 00:16:3e:92:f3:c1
  inc config device set "${ctPrefix}A" eth0 security.ipv6_filtering false
  rm "${INCUS_DIR}/networks/${brName}/dnsmasq.hosts/${ctPrefix}A.eth0"
  inc config device set "${ctPrefix}A" eth0 security.ipv6_filtering true
  inc start "${ctPrefix}A"
  ...
  inc init testimage "${ctPrefix}A" -p "${ctPrefix}"
  inc config device add "${ctPrefix}A" eth0 nic \
    name=eth0 \
    nictype=bridged \
    parent="${brName}" \
  ...
  inc network set "${brName}" ipv6.dhcp false
  inc network set "${brName}" ipv6.address none

  inc network set "${brName}" ipv6.dhcp.stateful false
  inc init testimage "${ctPrefix}A" -p "${ctPrefix}"
  inc config device add "${ctPrefix}A" eth0 nic \
    nictype=nic \
    name=eth0 \
  ...
  if inc config device unset "${ctPrefix}A" eth0 ipv4.address ; then
    echo "Shouldn't be able to unset IPv4 address with ipv4_filtering enabled and DHCPv4 disabled"
  fi

  if inc config device unset "${ctPrefix}A" eth0 ipv6.address ; then
  ...
  ! inc config device set "${ctPrefix}A" eth0 ipv4.address=192.0.2.2 || false
  ! inc config device set "${ctPrefix}A" eth0 ipv6.address=2001:db8::2 || false

  # Check IP filtering cannot be enabled without manual IP assigned in Incus config.
  ! inc config device set "${ctPrefix}A" eth0 security.ipv4_filtering=true || false
  inc config device set "${ctPrefix}A" eth0 ipv4.address=192.0.2.2 security.ipv4_filtering=true
  ! inc config device set "${ctPrefix}A" eth0 security.ipv6_filtering=true || false
    inc config device set "${ctPrefix}A" eth0 ipv6.address=2001:db8::2 security.ipv6_filtering=true
  ...
  # Check basic connectivity without any filtering.
  inc exec "${ctPrefix}A" -- ping -c2 -W5 192.0.2.1
  inc exec "${ctPrefix}A" -- ping -c2 -W5 2001:db8::1

  # Check fraudulent IPs are blocked.
  inc exec "${ctPrefix}A" -- ip a flush dev eth0
  inc exec "${ctPrefix}A" -- ip a add 192.0.2.3/24 dev eth0
  inc exec "${ctPrefix}A" -- ip a add 2001:db8::3/64 dev eth0

  ! inc exec "${ctPrefix}A" -- ping -c2 -W5 192.0.2.1 || false
  ! inc exec "${ctPrefix}A" -- ping -c2 -W5 2001:db8::1 || false

  # Check IP filtering can be enabled with IP assigned as none in Incus config.
  inc config device set "${ctPrefix}A" eth0 ipv4.address=none security.ipv4_filtering=true
  inc config device set "${ctPrefix}A" eth0 ipv6.address=none security.ipv6_filtering=true
  inc exec "${ctPrefix}A" -- ip a flush dev eth0
  inc exec "${ctPrefix}A" -- ip a add 192.0.2.2/24 dev eth0
  inc exec "${ctPrefix}A" -- ip a add 2001:db8::2/64 dev eth0
  ! inc exec "${ctPrefix}A" -- ping -c2 -W5 192.0.2.1 || false
