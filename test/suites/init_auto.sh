test_init_auto() {
  ...
  if [ "$(storage_backend "$INCUS_DIR")" = "zfs" ]; then
    ...
    INCUS_DIR=${INCUS_INIT_DIR} lxd init --auto --storage-backend zfs --storage-pool "incustest-$(basename "${INCUS_DIR}")-pool1-existing-pool"
    INCUS_DIR=${INCUS_INIT_DIR} lxc profile show default | grep -q "pool: default"
    ...
    configure_loop_device loop_file_1 loop_device_1
    zpool create -m none -O compression=on "incustest-$(basename "${INCUS_DIR}")-pool1-existing-pool" "${loop_device_1}"
    INCUS_DIR=${INCUS_INIT_DIR} lxd init --auto --storage-backend zfs --storage-pool "incustest-$(basename "${INCUS_DIR}")-pool1-existing-pool/non-existing-dataset"
    kill_incus "${INCUS_INIT_DIR}"
    ...
    INCUS_DIR=${INCUS_INIT_DIR} lxd init --auto --storage-backend zfs --storage-pool "incustest-$(basename "${INCUS_DIR}")-pool1-existing-pool/existing-dataset"
  ...
  INCUS_DIR=${INCUS_INIT_DIR} lxd init --trust-password test --network-address 127.0.0.1 --network-port "$(local_tcp_port)" --auto
