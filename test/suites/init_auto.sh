test_init_auto() {
  # - incusd init --auto --storage-backend zfs
  # and
  # - incusd init --auto
  ...
  if [ "$(storage_backend "$INCUS_DIR")" = "zfs" ]; then
    # incusd init --auto --storage-backend zfs --storage-pool <name>
    ...
    # incusd init --auto --storage-backend zfs --storage-pool <name>/<non-existing-dataset>
    INCUS_INIT_DIR=$(mktemp -d -p "${TEST_DIR}" XXX)
    chmod +x "${INCUS_INIT_DIR}"
    spawn_incus "${INCUS_INIT_DIR}" false
    ...
    configure_loop_device loop_file_1 loop_device_1
    zpool create -m none -O compression=on "incustest-$(basename "${INCUS_DIR}")-pool1-existing-pool" "${loop_device_1}"
    INCUS_DIR=${INCUS_INIT_DIR} incusd init --auto --storage-backend zfs --storage-pool "incustest-$(basename "${INCUS_DIR}")-pool1-existing-pool/non-existing-dataset"
    kill_incus "${INCUS_INIT_DIR}"

    # incusd init --auto --storage-backend zfs --storage-pool <name>/<existing-dataset>
    INCUS_INIT_DIR=$(mktemp -d -p "${TEST_DIR}" XXX)
    chmod +x "${INCUS_INIT_DIR}"
    spawn_incus "${INCUS_INIT_DIR}" false

    zfs create -p -o mountpoint=none "incustest-$(basename "${INCUS_DIR}")-pool1-existing-pool/existing-dataset"
    INCUS_DIR=${INCUS_INIT_DIR} incusd init --auto --storage-backend zfs --storage-pool "incustest-$(basename "${INCUS_DIR}")-pool1-existing-pool/existing-dataset"
    ...
    # incusd init --storage-backend zfs --storage-create-loop 1 --storage-pool <name> --auto
    INCUS_INIT_DIR=$(mktemp -d -p "${TEST_DIR}" XXX)
    chmod +x "${INCUS_INIT_DIR}"
    spawn_incus "${INCUS_INIT_DIR}" false
