test_storage_volume_attach() {
  ...
  lxc storage volume create "incustest-$(basename "${INCUS_DIR}")" testvolume
  ...
  lxc storage volume attach "incustest-$(basename "${INCUS_DIR}")" testvolume c1 testvolume
  ...
  if [ "${UIDs}" -lt 500000 ] || [ "${GIDs}" -lt 500000 ]; then
    ...
    lxc rm -f c1 c2
    lxc storage volume delete "incustest-$(basename "${INCUS_DIR}")" testvolume
    return
  fi
  ...
  ! lxc storage volume attach "incustest-$(basename "${INCUS_DIR}")" testvolume c2 testvolume || false
  ...
  lxc storage volume attach "incustest-$(basename "${INCUS_DIR}")" testvolume c2 testvolume
