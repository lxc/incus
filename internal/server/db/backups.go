...
func (c *Cluster) GetStoragePoolBucketBackupsName(projectName string, bucketName string) ([]string, error) {
	var result []string
...
func (c *Cluster) CreateStoragePoolBucketBackup(args StoragePoolBucketBackup) error {
	_, err := c.getStoragePoolBucketBackupID(args.Name)
	if err == nil {
		return ErrAlreadyDefined
	}
...
func (c *Cluster) getStoragePoolBucketBackupID(name string) (int, error) {
...
func (c *Cluster) DeleteStoragePoolBucketBackup(name string) error {
	id, err := c.getStoragePoolBucketBackupID(name)
	if err != nil {
		return err
	}

	err = c.Transaction(context.TODO(), func(ctx context.Context, tx *ClusterTx) error {
		_, err := tx.tx.ExecContext(ctx, "DELETE FROM storage_buckets_backups WHERE id=?", id)
		return err
	})
	if err != nil {
		return err
...
func (c *Cluster) GetStoragePoolBucketBackup(projectName string, poolName string, backupName string) (StoragePoolBucketBackup, error) {
...
// RenameVolumeBackup renames a volume backup from the given current name
// to the new one.
func (c *Cluster) RenameBucketBackup(oldName, newName string) error {
	err := c.Transaction(context.TODO(), func(ctx context.Context, tx *ClusterTx) error {
