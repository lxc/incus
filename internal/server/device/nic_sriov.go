...
func (d *nicSRIOV) Start() (*deviceConfig.RunConfig, error) {
	...
	d.logger.Error("Parent", logger.Ctx{"parent": parent})
...
func (d *nicSRIOV) findParent() (string, error) {
	...
	d.logger.Error("NUMA", logger.Ctx{"set": numaNodeSet, "fallback": numaNodeSetFallback})
	...
	for _, nic := range interfaces.Cards {
		...
		d.logger.Error("Selected NIC", logger.Ctx{"PCI": nic.PCIAddress, "VendorID": nic.VendorID, "ProductID": nic.ProductID})

		// Skip any card without SR-IOV.
		if nic.SRIOV == nil {
			d.logger.Error("SR-IOV nil")
			continue
		}
		...
		for _, port := range nic.Ports {
			...
			d.logger.Error("VFs", logger.Ctx{"Port": port.ID, "freeVf": freeVf, "totalVf": totalVf})
		}
		...
		if currentVfFreeRatio == 0 {
			d.logger.Error("no available VFs")
			continue
		}
		...
		if parent == "" || currentVfFreeRatio > vfFreeRatio {
			d.logger.Error("Saving card", logger.Ctx{"PCI": nic.PCIAddress})
			parent = currentParent
			vfFreeRatio = currentVfFreeRatio
			cardNUMA = int(nic.NUMANode)
...
func nicSelected(device deviceConfig.Device, nic api.ResourcesNetworkCard) bool {
	return !((device["vendorid"] != "" && nic.VendorID != device["vendorid"]) ||
		(device["productid"] != "" && nic.ProductID != device["productid"]) ||
		(device["pci"] != "" && nic.PCIAddress != device["pci"]))
}
