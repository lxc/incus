	...
	if len(idmaps) > 0 {
		...
		for _, entry := range idmapSet.Entries {
			if entry.IsUID {
				...
				args = append(args, fmt.Sprintf("--translate-uid=forbid-guest:%d:%d", lastUID, entry.NSID-1))
				lastUID = entry.NSID
			}
			...
			if entry.IsGID {
				...
				args = append(args, fmt.Sprintf("--translate-gid=forbid-guest:%d:%d", lastGID, entry.NSID-1))
				lastGID = entry.NSID
			}
		}

		if lastUID < 4294967295 {
			args = append(args, fmt.Sprintf("--translate-uid=forbid-guest:%d:%d", lastUID+1, 4294967295))
		}

		if lastGID < 4294967295 {
			args = append(args, fmt.Sprintf("--translate-gid=forbid-guest:%d:%d", lastGID+1, 4294967295))
		}
	}
