...
func NewBucketBackup(state *state.State, projectName, poolName, bucketName string, ID int, name string, creationDate, expiryDate time.Time) *BucketBackup {
	return &BucketBackup{
...
func (b *BucketBackup) Delete() error {
	...
	err := b.state.DB.Cluster.DeleteStoragePoolBucketBackup(b.name)
	if err != nil {
		return err
	}
...
func (b *BucketBackup) Rename(newName string) error {
	...
	err = b.state.DB.Cluster.RenameBucketBackup(b.name, newName)
	if err != nil {
		return err
	}
