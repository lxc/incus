...
func (n *ovn) localPeerCreate(peer api.NetworkPeersPost) error {
	...
	err := n.state.DB.Cluster.Transaction(ctx, func(ctx context.Context, tx *db.ClusterTx) error {
		...
		filter := dbCluster.NetworkPeerFilter{NetworkID: &n.id, Name: &peer.Name}
		...
		if len(dbPeers) == 0 {
			return fmt.Errorf("Network peer %q not found", peer.Name)
		}

		if len(dbPeers) > 1 {
			return fmt.Errorf("Multiple network peers found for name %q", peer.Name)
		}
...
func (n *ovn) PeerCreate(peer api.NetworkPeersPost) error {
	...
	err = n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error { // Create peer DB record.
		record := dbCluster.NetworkPeer{
			...
			// FIXME fix this logic and peering state logic.
			TargetNetworkProject:       &peer.TargetNetworkProject,
			TargetNetworkName:          &peer.TargetNetworkName,
			TargetNetworkIntegrationID: &peer.TargetNetworkIntegrationID,
			TargetNetworkID:            &peer.TargetNetworkID,
		}
...
func (n *ovn) PeerUpdate(peerName string, req api.NetworkPeerPut) error {
	...
	err := n.state.DB.Cluster.Transaction(context.TODO(), func(ctx context.Context, tx *db.ClusterTx) error {
		filter := dbCluster.NetworkPeerFilter{NetworkID: &n.id, Name: &peerName}
		dbPeers, err := dbCluster.GetNetworkPeers(ctx, tx.Tx(), filter)
		if err != nil {
			return fmt.Errorf("Failed getting network peer DB object: %w", err)
		}
		...
		if len(dbPeers) == 0 {
			return fmt.Errorf("Network peer %q not found", peerName)
		}
		...
		dbPeer := dbPeers[0]
		curPeerID = dbPeer.ID
		curPeer, err = dbPeer.ToAPI(ctx, tx.Tx())
		if err != nil {
			return fmt.Errorf("Failed converting network peer DB object to API object: %w", err)
		}
