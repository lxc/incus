...
func (b *backend) DeleteInstanceSnapshot(inst instance.Instance, op *operations.Operation) error {
	...
	if volExists {
		if srcDBVol.Config["block.type"] == drivers.BlockVolumeTypeQcow2 {
			randomSuffix, err := internalUtil.RandomHexString(10)
			if err != nil {
				return fmt.Errorf("Failed creating temporary file name: %w", err)
			}

			parent, _, _ := api.GetParentAndSnapshotName(vol.Name())
			tmpSnapName := fmt.Sprintf("%s_%s", parent, randomSuffix)
			...
			parentVol := b.GetVolume(volType, contentType, parentName, srcDBVol.Config)
			err = b.qcow2DeleteSnapshot(parentVol, vol, tmpVol, inst, op)
			if err != nil {
				return err
			}
...
func (b *backend) qcow2DeleteSnapshot(vol drivers.Volume, snapVol drivers.Volume, tmpVol drivers.Volume, inst instance.Instance, op *operations.Operation) error {
	// Return if this is not a qcow2 image.
	if vol.Config()["block.type"] != drivers.BlockVolumeTypeQcow2 {
		return nil
	}
	...
	err = vol.MountWithSnapshotsTask(func(mountPath string, op *operations.Operation) error {
		...
		for _, snap := range volSnaps {
			...
			if snapName == currentSnapName {
				baseVol = imgInfo.BackingFilename
			}
		}
		...
		err = drivers.Qcow2Convert(destinationVol, baseVol, tmpVolDiskPath)
		if err != nil {
			return err
		}
