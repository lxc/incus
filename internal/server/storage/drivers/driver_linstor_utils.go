...
func (d *linstor) createVolumeSnapshot(parentVol Volume, snapVol Volume) error {
	...
	err = linstor.Client.ResourceDefinitions.Modify(context.TODO(), resourceName, linstorClient.GenericPropsModify{
		OverrideProps: map[string]string{"Aux/Incus/SnapshotName/" + snapResourceName: snapshotName},
	})
	if err != nil {
...
func (d *linstor) deleteVolumeSnapshot(parentVol Volume, snapVol Volume) error {
	...
	return linstor.Client.ResourceDefinitions.Modify(context.TODO(), resourceName, linstorClient.GenericPropsModify{
		DeleteProps: []string{"Aux/Incus/SnapshotName/" + snapResourceName},
	})
}
...
func (d *linstor) getSnapshotMap(parentVol Volume) (map[string]string, error) {
	...
	for key, value := range resourceDefinition.Props {
		if strings.HasPrefix(key, "Aux/Incus/SnapshotName/") {
			result[strings.TrimPrefix(key, "Aux/Incus/SnapshotName/")] = value
		}
	}
